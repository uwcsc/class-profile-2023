...
export const BarGraphHorizontal: React.FC<BarGraphProps> = ({
  width,
  height,
  margin,
  data,
            ...
            {data.map((d, idx) => {
              ...
              return (
                <Group className={styles.barGroup} key={`bar-${barName}`}>
                  <Bar className={`${styles.bar} ${styles[color]}`} x={0} y={categoryPoint(d)} width={barLength} height={barWidth} />
                  <text y={(categoryPoint(d) ?? 0) + barWidth / 2} dominantBaseline="middle" x={barLength + (barLength < 50 ? 10 : -32)}>
                    {d.value}
                  </text>
                </Group>
              );
...
export const BarGraphVertical: React.FC<BarGraphProps> = ({
  width,
  height,
  margin,
  data,
        ...
        <Group top={margin.top} left={margin.left}>
          ...
          <Group>
            {data.map((d, idx) => {
              ...
              return (
                <Group className={styles.barGroup} key={`bar-${barName}`}>
                  <Bar className={`${styles.bar} ${styles[color]}`} x={categoryPoint(d)} y={valueMax - barHeight} width={barWidth} height={barHeight} />
                  <text x={(categoryPoint(d) ?? 0) + barWidth / 2} textAnchor="middle" y={valueMax - barHeight + (barHeight < 50 ? -10 : 32)}>
                    {d.value}
                  </text>
                </Group>
              );
